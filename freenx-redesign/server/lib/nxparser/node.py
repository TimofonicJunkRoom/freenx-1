...
class parser(nxparser.base.parser):
    ...
    class node_session:
        ...
        def __init__(self, id, args):
            ...
            self.name = self.args.get(
                'session', "%s:%s" % (self.hostname, self.display))
    ...
    def __init__(
        self, input, output, version=nxparser.base.parser.DEFAULT_VERSION,
        ...
        """
        nxparser.base.parser.__init__(
            self, input, output, version=version, program=program)

    def banner(self):
        """Write the protocol banner to the output."""

        self.prompt(1000, '%s - Version %s' %
                    (self.program.upper(), self.version))
